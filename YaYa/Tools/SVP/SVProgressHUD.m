...
+ (void)setLocationType:(SVProgressHUDLocationType)locationType{
    ...
    switch (locationType) {
        case SVProgressHUDLocationTop:
            [SVProgressHUD setOffsetFromCenter:UIOffsetMake(0, -(UIScreen.mainScreen.bounds.size.height/2 - 160))];
            break;
        case SVProgressHUDLocationMiddle:
            break;
        case SVProgressHUDLocationBottom:
            [SVProgressHUD setOffsetFromCenter:UIOffsetMake(0, UIScreen.mainScreen.bounds.size.height/2 - 83)];
            break;
        default:
            break;
    }
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != SVProgressHUDMaskTypeNone) {
        ...
        self.accessibilityLabel = self.statusLabel.text;
        self.isAccessibilityElement = YES;
    } else {
        self.controlView.userInteractionEnabled = NO;
        self.hudView.accessibilityLabel = self.statusLabel.text;
